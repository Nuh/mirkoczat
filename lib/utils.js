...
let utils = {
    ...
    proxy: {
        ...
        eventWith: (source, target, events, interceptor) => {
            return _([...events]).flattenDeep().each((e) => source.on(e, (...args) => target.emit(e, ...interceptor(args))));
        }
    },
    convert: {
    ...
    extract: {
        username: (user) => user && user.username ? user.username : user || 'SYSTEM'
    }
};
